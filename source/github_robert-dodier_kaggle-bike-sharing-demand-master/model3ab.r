...
# model 3ab: estimate = mean of log(target + 1) by hour of day, day type,
...
for (i in 1:24) {
  ...
  lm3ab[[i]] <- lm (model3.residuals ~ 1 + days.elapsed + I(days.elapsed^2), data=train.train.with.new.fields[filter,])
}
...
for (i in 1:24) {
  ...
  predict.residual.in.sample[filter] <- predict (lm3ab[[i]], train.train.with.new.fields[filter,])
}
...
for (i in 1:24) {
  ...
  predict.residual.out.of.sample[filter] <- predict (lm3ab[[i]], train.test.with.new.fields[filter,])
}
