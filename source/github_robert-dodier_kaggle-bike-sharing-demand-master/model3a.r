...
lm3a <- lm (model3.residuals ~ atemp + casual + holiday + humidity + registered + season + temp + weather + windspeed,
             data=train.train.with.new.fields)

summary (lm3a)

lm3a.prediction.in.sample <- predict (lm3a, train.train)

lm3a.prediction.in.sample.combined <- exp (log (in.sample.estimate + 1) + lm3a.prediction.in.sample) - 1

rmsle (train.train$count, lm3a.prediction.in.sample.combined)

lm3a.prediction.out.of.sample <- predict (lm3a, train.test)

lm3a.prediction.out.of.sample.combined <- exp (log (out.of.sample.estimate + 1) + lm3a.prediction.out.of.sample) - 1

rmsle (train.test$count, lm3a.prediction.out.of.sample.combined)
