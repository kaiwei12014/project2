...
for (i in 1:24) {
  ...
  lm3ac[[i]] <- lm (model3.residuals ~ days.elapsed  + atemp + humidity + temp + weather + windspeed,
                    data=train.with.new.fields[filter,])
}
...
for (i in 1:24) {
  ...
  predict.residual.in.sample[filter] <- predict (lm3ac[[i]], train.with.new.fields[filter,])
}
...
for (i in 1:24) {
  ...
  predict.residual.out.of.sample[filter] <- predict (lm3ac[[i]], test.with.new.fields[filter,])
}
...
write.table (test.predict, "test.predict.model3ac.csv", sep=",", quote=F, row.names=F)
