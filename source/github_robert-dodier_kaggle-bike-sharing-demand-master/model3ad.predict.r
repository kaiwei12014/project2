...
for (i in 1:24) {
  ...
  randomForest3ad[[i]] <- randomForest (model3.residuals ~ days.elapsed  + atemp + humidity + temp + weather + windspeed,
                    data=train.with.new.fields[filter,], mtry=3, importance=T)
...
for (i in 1:24) {
  ...
  predict.residual.in.sample[filter] <- predict (randomForest3ad[[i]], train.with.new.fields[filter,])
}
...
for (i in 1:24) {
  ...
  predict.residual.out.of.sample[filter] <- predict (randomForest3ad[[i]], test.with.new.fields[filter,])
}

predict.out.of.sample.combined <- exp (log (out.of.sample.estimate + 1) + predict.residual.out.of.sample) - 1

negative.predictions <- predict.out.of.sample.combined < 0

if (sum (negative.predictions) > 0) {
  print (sprintf ("There are %d negative predictions; set them to zero.", sum (negative.predictions)))
  predict.out.of.sample.combined [negative.predictions] <- 0
}

test.predict <- data.frame (datetime=test$datetime, count=predict.out.of.sample.combined)

write.table (test.predict, "test.predict.model3ad.csv", sep=",", quote=F, row.names=F)
