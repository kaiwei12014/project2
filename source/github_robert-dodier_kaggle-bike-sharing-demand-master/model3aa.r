...
# model 3aa: estimate = mean of log(target + 1) by hour of day, day type,
# then model residuals of that as residual log(target + 1) ~ quadratic fitted to recent residuals
# (next step in model refinement is to weight recent residuals more heavily)
...
# lm3aa <- lm (model3.residuals ~ atemp + casual + holiday + humidity + registered + season + temp + weather + windspeed,
...
for (i in 1:24) {
  ...
  lm3aa[[i]] <- lm (model3.residuals ~ days.elapsed, data=train.train.with.new.fields[filter,])
}
...
for (i in 1:24) {
  ...
  predict.residual.in.sample[filter] <- predict (lm3aa[[i]], train.train.with.new.fields[filter,])
}
...
for (i in 1:24) {
  ...
  predict.residual.out.of.sample[filter] <- predict (lm3aa[[i]], train.test.with.new.fields[filter,])
}
